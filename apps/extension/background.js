...
// Add this chrome runtime message listener
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === "JWT_RECEIVED") {
    const jwt = message.token;
    console.log("Received JWT:", jwt);

    // Check if we're on an allowed domain
    if (
      sender.url.startsWith("http://localhost:") ||
      sender.url.startsWith("https://supermemory.ai") ||
      sender.url.startsWith("https://beta.supermemory.ai")
    ) {
            ...
            if (jwt) {
              for (const tweet of parsedTweets) {
                await fetch(`${BACKEND_URL}/api/store`, {
                  ...
                  body: JSON.stringify({
                    pageContent: tweet.full_text,
                    url: `https://twitter.com/web/status/${tweet.id}`,
              ...
              console.error("JWT not found in storage");
            }
          });
...
const parseTweet = (entry) => {
  ...
  return {
    id: entry.entryId.replace('tweet-', ''),
